...
public class Map {
     ...
     * @throws Exception If it couldn't find a solution
     */
    public Hashtable<String, Integer> getGreedyMapColoring() throws Exception {
        ...
        Hashtable<String, Integer> finalColoration = new Hashtable<>(v);
     ...
     * @throws Exception    If it couldn't find a solution
     */
    private void bfsColor(int startPoint, boolean[] availableColor, int[] result) throws Exception {
        ...
        while (!queue.isEmpty()) {
            ...
            if (result[currNode] == -1)
                throw new Exception("Greedy Algorithm couldn't find a solution using only 4 colors");
     ...
     * @throws Exception If the graph couldn't be painted (This is theoretically impossible because of the 4 color theorem)
     */
    public Hashtable<String, Integer> getBacktrackingMapColoring() throws Exception {
        int availableColors = 4;
        int currentNodeIndex = 0;
        int[] result = new int[v];
        Arrays.fill(result, -1);
        if (!backtrackingColor(result, currentNodeIndex, availableColors))
            throw new Exception("Back tracking algorithm couldn't find a solution with only 4 colors, this means that the graph does not represents a map");
        Hashtable<String, Integer> finalColoration = new Hashtable<>(v);
        for (int i = 0; i < result.length; i++) {
            finalColoration.put(indexToName[i], result[i]);
        }
    ...
    private boolean notAssigned(int currNode, int color, int[] resultSoFar) {
        LinkedList<Integer> neighbors = adj[currNode];
        for (Integer neighbor : neighbors) {
